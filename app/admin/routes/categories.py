...
def add_new_category():
    try:
        if request.method.__eq__('POST'):
            ...
            # Xử lý file upload
            file = request.form['thumbnail']
            print("Name:", name)  # Debug: In ra tên chuyên mục
            print("Thumbnail URL:", file)  # Debug: In ra URL của thumbnail
            # filename = handle_image_upload(file)

            # Nếu không có file URL thì có thể trả về lỗi custom hoặc xử lý khác
            if not file:
                flash('No thumbnail selected!', 'danger')
                return redirect(url_for('admin.add_new_category'))

            new_category = Category(
                name=name,
                thumbnail=file  # Lưu tên file vào database
            )
        ...
        logging.error(f"Error while adding new category: {e}")
